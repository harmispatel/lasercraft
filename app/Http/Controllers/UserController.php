...
class UserController extends Controller
{
    ...
    // Verify Clients
    function clientsVerify(Request $request)
    {
        $verification = $request->verification;
        $user_id = $request->id;
        $message = ($verification == 1) ? 'User has been Verified SuccessFully..' : 'User has been Unverified SuccessFully..';

        try
        {
            $client = User::find($user_id);
            $client->user_verify = $verification;
            $client->update();
