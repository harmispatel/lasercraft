@php
    ...
    $top_logo = '<img src="data:image/png;base64,'.$top_logo.'" width="80">';

    $currency = (isset($datails['currency'])) ? $datails['currency'] : 'AUD';

    $order_details = (isset($details['order_details'])) ? $details['order_details'] : '';
    $order_status = (isset($details['order_status'])) ? $details['order_status'] : '';
    ...
    $mail_format = str_replace('{order_status}',$order_status,$mail_format);
            ...
            $order_html .= '<tbody style="font-weight: 600!important;">';
                if(count($order_items) > 0)
                {
                    foreach($order_items as $order_item)
                    {
                        ...
                        $item_image = (count($item_images) > 0  && isset($item_images[0]['image']) && !empty($item_images[0]['image']) && file_exists(public_path('client_uploads/items/'.$item_images[0]['image']))) ? public_path('client_uploads/items/'.$item_images[0]['image']) : public_path('client_images/not-found/no_image_1.jpg');
